...
func (i *IcmpFwd) sendPkt(pkt stack.PacketBufferPtr) bool {
	...
	if pkt.NetworkProtocolNumber == ipv4.ProtocolNumber {
		...
		if err != nil {
			klog.Error("error writing to icmp4 socket", err)
			return false
		}
		return true
		...
		if err != nil {
			klog.Error("error writing to icmp6 socket", err)
			return false
		}
		return true
	}
...
func (i *IcmpFwd) forwardReplies4() error {
	...
	for {
		...
		if err != nil {
			klog.Error("error reading from icmp4 socket", err)
			return err
		}
		msg := fullBuf[:n]
		...
		if err != nil {
			klog.Error("error handling icmp4 reply", err)
		}
	}
}
...
func (i *IcmpFwd) forwardReplies6() error {
	...
	for {
		...
		if err != nil {
			klog.Error("error reading from icmp6 socket", err)
			return err
		}
		msg := fullBuf[:n]
		...
		if err != nil {
			klog.Error("error handling icmp6 reply", err)
		}
	}
}
