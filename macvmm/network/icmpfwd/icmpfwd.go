...
func Handle(s *stack.Stack) {
	...
	tid := stack.NATID
	PushRule(s, rule4, tid, false)
	PushRule(s, rule6, tid, true)
...
func sendEchoResponse(s *stack.Stack, packet stack.PacketBufferPtr) {
	...
	tcpipErr := SendPacket(s, append(ipHeader, response...), &tcpip.FullAddress{NIC: 1, Addr: netHeader.SourceAddress()}, netProto)
	if tcpipErr != nil {
