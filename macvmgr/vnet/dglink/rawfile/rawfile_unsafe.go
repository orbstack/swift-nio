...
func NonBlockingWrite(fd int, buf []byte) tcpip.Error {
	_, e := unix.Write(fd, buf)
	if e != nil {
		return TranslateErrno(e.(syscall.Errno))
	}
...
func BlockingPollUntilStopped(efd int, fd int, events int16) (bool, unix.Errno) {
	...
	_, errno := unix.Poll(pfds[:], -1)
	if errno != nil {
		return pfds[0].Revents&unix.POLLIN != 0, errno.(unix.Errno)
	}

	if pfds[1].Revents&unix.POLLHUP != 0 || pfds[1].Revents&unix.POLLERR != 0 {
