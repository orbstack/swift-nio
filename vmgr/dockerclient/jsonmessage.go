...
// NOTE: modified from https://github.com/moby/moby/blob/fc68ffcb184babeda9e139466a588a70ca9ce716/pkg/jsonmessage/jsonmessage.go#L1
// this file is used for streaming progress messages, such as image pull progress
...
// HumanSize copied from https://github.com/docker/go-units/blob/16e18b2861ca6fd622e7042ffeb9a3ebe8a9dff9/size.go#L1
const (
	// Decimal
	KB = 1000
	MB = 1000 * KB
...
func HumanSize(size float64) string {
	...
	return fmt.Sprintf("%.*g%s", 4, size, unit)
}

// End copied HumanSize implementation
...
func (p *JSONProgress) String() string {
	...
	if p.Total <= 0 {
		switch p.Units {
		case "":
			return fmt.Sprintf("%8v", HumanSize(float64(p.Current)))
		default:
	...
	case p.Units == "": // no units, use bytes
		current := HumanSize(float64(p.Current))
		total := HumanSize(float64(p.Total))
...
func (p *JSONProgress) width() int {
	...
	w, _, err := term.GetSize(int(p.terminalFd))
	if err == nil {
		return w
	}
	return 200
