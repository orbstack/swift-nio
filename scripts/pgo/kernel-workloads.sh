...
orb -w "/home/$USER/linux" sudo hyperfine -p 'echo 3 > /proc/sys/vm/drop_caches' 'rg hvc_put; fd -uuu'

# parallel ripgrep, cached
orb -w "/home/$USER/linux" hyperfine 'rg hvc_put; fd -uuu'

# serial ripgrep, cached
orb -w "/home/$USER/linux" hyperfine 'rg -j1 hvc_put; fd -j1 -uuu'

# hackbench
orb -w "/home/$USER" hyperfine 'rt-tests/hackbench -pTl 8000'

# kernel build
# this also stresses madvise(MADV_DONTNEED) thanks to jemalloc LD_PRELOAD
# and mprotect contention
orb -s 'cd ~/linux && source setup.sh && rm -fr o2 && mkdir o2 && cp configs/arm64 o2/.config && marm O=o2'

# pnpm on virtiofs
cd ~/code/projects/macvirt/exp/fsbench/npm
# arch linux arm pnpm is broken (incompatible node.js version)
# run twice: one uncached, one cached
docker run -i --rm -v $PWD:/data -w /data node sh -c 'rm -fr node_modules && npm install -g pnpm && pnpm i && rm -fr node_modules && pnpm i'
...
# ripgrep on virtiofs
cd node_modules
orb sudo hyperfine -p 'echo 3 > /proc/sys/vm/drop_caches' 'rg webp; fd -uuu'

# bpftool on virtiofs
cd /tmp
orb sudo hyperfine "bpftool btf dump file /sys/kernel/btf/vmlinux format c > bpftool.txt"

# docker container startup
hyperfine "docker run -i --rm -v $PWD:/data alpine echo"

# procfs
orb hyperfine 'ps awux'

# machine restart
hyperfine 'orb restart ubuntu'

# machine create and delete (squashfs)
orb delete -f pgo-ubuntu-test || :
orb create ubuntu pgo-ubuntu-test
orb delete -f pgo-ubuntu-test

# network in machines
# iperf, ipv4, TCP, send
orb iperf3 -c host.orb.internal -4
# iperf, ipv4, TCP, receive
orb iperf3 -c host.orb.internal -4 -R
# iperf, ipv6, TCP, send
orb iperf3 -c host.orb.internal -6
# iperf, ipv6, TCP, receive
orb iperf3 -c host.orb.internal -6 -R
# iperf, ipv4, UDP, send
orb iperf3 -c host.orb.internal -4 -u -b 2000M
# iperf, ipv4, UDP, receive
orb iperf3 -c host.orb.internal -4 -u -R -b 2000M
# iperf, ipv6, UDP, send
orb iperf3 -c host.orb.internal -6 -u -b 2000M
# iperf, ipv6, UDP, receive
orb iperf3 -c host.orb.internal -6 -u -R -b 2000M
...
# pgbench on btrfs
cd ~/code/vm/fsbench/pgbench
make db
make pgbench
make clean

# pgbench on virtiofs
cd ~/code/projects/macvirt/exp/fsbench/pgbench
make db
make pgbench
make clean

# docker image build with rosetta
cd ~/code/projects/macvirt/rootfs
docker builder prune -a -f
make amd64-debug

# NFS server
cd ~/OrbStack/arch/home/$USER/linux
hyperfine 'rg hvc_put'

# memory pressure: run until OOM, 2x
cd ~/code/projects/macvirt
orb exp/mem/memhog-linux
orb exp/mem/memhog-linux

# MGLRU aging + vfs cache pressure
# disable tty, so we can background this
ssh ovm apk add hyperfine
orb -w "/home/$USER/linux" hyperfine 'rg -j1 hvc_put; fd -j1 -uuu' 2>&1 </dev/null | cat &
sleep 1
...
# internally in VM
orb hey -z 10s http://docker.orb.internal:8123
docker stop ngbench
...
orb -w /home/$USER bash ~/code/projects/macvirt/exp/benchmarks/disk/fio.sh

# virtiofs clone with tiny reads/writes
cd /Volumes/CaseSensitive
rm -fr linux-ref-clone2
orb git clone linux-ref-clone linux-ref-clone2
rm -fr linux-ref-clone2
...
cd ~/code/projects/macvirt/vinit
rm -fr target
orb cargo build
...
echo 3 | orb sudo tee /proc/sys/vm/drop_caches
