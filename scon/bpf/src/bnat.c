...
// route (i.e. us) also, it needs to be RFC private IP due to applications then we use a static ip
// route to redirect it to eth1 (where our egress4 prog is attached)
//
// ideally we'd MASQUERADE this to Docker bridge gateway IP in Docker machine, but we need to make
// sure cfwd can differentiate it and make sk_lookup hook kick in I tried to mod the kernel and add
// mark to "struct bpf_sk_lookup" but it doesn't work b/c skb is usually NULL at the lookup point.
// so instead, use a weird random private IP to make private RFC IP checks like Keycloak happy,
// while minimizing chance of conflict marks are lost across bridge too, but i'm sure there's a way
// around that
//
...
// this is the xlat-mapped version of NAT64_SRC_IP4 source addr. that way, we get full checksum
...
static int icmp4_to_icmp6(struct __sk_buff *skb, int nh_off) {
    ...
    if (bpf_skb_load_bytes(skb, nh_off, &icmp4, sizeof(icmp4)) < 0) return -1;
    ...
    if (bpf_skb_store_bytes(skb, nh_off, &icmp6, sizeof(icmp6), 0) < 0) return -1;
    icmp4.checksum = 0;
    icmp6.icmp6_cksum = 0;
...
static int icmp6_to_icmp4(struct __sk_buff *skb, int nh_off) {
    ...
    if (bpf_skb_load_bytes(skb, nh_off, &icmp6, sizeof(icmp6)) < 0) return -1;
    ...
    if (bpf_skb_store_bytes(skb, nh_off, &icmp4, sizeof(icmp4), 0) < 0) return -1;
    icmp4.checksum = 0;
    icmp6.icmp6_cksum = 0;
...
int sched_cls_ingress6_nat6(struct __sk_buff *skb) {
    ...
    if (ip6->nexthdr == IPPROTO_ICMPV6) {
        ...
        __be32 csum1 = ipv6_pseudohdr_checksum(ip6, IPPROTO_ICMPV6, bpf_ntohs(ip6->payload_len), 0);
    ...
    *(struct iphdr *)(eth + 1) = ip;
    ...
    if (ip.protocol == IPPROTO_ICMP) {
        if (bpf_l4_csum_replace(skb, ETH_HLEN + sizeof(ip) + offsetof(struct icmphdr, checksum), 0,
                                l4csum, BPF_F_PSEUDO_HDR)) {
...
int sched_cls_egress4_nat4(struct __sk_buff *skb) {
    ...
    if ((void *)(ip4 + 1) > data_end) return TC_ACT_PIPE;

    // only if translated
    // do this early so we exit fast for pure v4 traffic
    if (ip4->daddr != NAT64_SRC_IP4) return TC_ACT_PIPE;

    // drop packet on any failure after subnet check. it'll be wrong anyway

    // IP version must be 4
    if (ip4->version != 4) return TC_ACT_SHOT;

    // We cannot handle IP options, just standard 20 byte == 5 dword minimal IPv4 header
    if (ip4->ihl != 5) return TC_ACT_SHOT;

    // Maximum IPv6 payload length that can be translated to IPv4
    if (bpf_htons(ip4->tot_len) > 0xFFFF - sizeof(struct iphdr)) return TC_ACT_SHOT;

    // Minimum IPv4 total length is the size of the header
    if (bpf_ntohs(ip4->tot_len) < sizeof(*ip4)) return TC_ACT_SHOT;

    // We are incapable of dealing with IPv4 fragments
    if (ip4->frag_off & ~bpf_htons(IP_DF)) return TC_ACT_SHOT;
    ...
    case IPPROTO_UDP: // See above comment, but must also have UDP header...
        if (data + ETH_HLEN + sizeof(*ip4) + sizeof(struct udphdr) > data_end) return TC_ACT_SHOT;
    ...
    if (ip4->protocol == IPPROTO_ICMP) {
        ...
        if (bpf_l4_csum_replace(skb,
                                ETH_HLEN + sizeof(*ip4) + offsetof(struct icmp6hdr, icmp6_cksum), 0,
                                l4csum, BPF_F_PSEUDO_HDR)) {
    ...
    // Packet mutations begin - point of no return, but if this first modification fails
    // the packet is probably still pristine, so let clatd handle it.
    if (bpf_skb_change_proto(skb, bpf_htons(ETH_P_IPV6), 0)) return TC_ACT_SHOT;

    // bpf_skb_change_proto() invalidates all pointers - reload them.
    data = (void *)(long)skb->data;
    data_end = (void *)(long)skb->data_end;
    eth = data;
    if (data + ETH_HLEN + sizeof(ip6) > data_end) return TC_ACT_SHOT;
