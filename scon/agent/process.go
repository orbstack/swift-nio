	...
	if args.User != "" {
		...
		if args.ReplaceShell && args.CombinedArgs[0] == ShellSentinel {
			...
			if args.DoLogin {
				...
				// set a default LANG
				// inheriting LANG from host causes too many issues (e.g. with ncurses, perl) if the locale isn't available in the machine.
				// although C.UTF-8 may not be the right language, it's guaranteed to cause almost no issues, and always works out of the box.
				// setting a default like en_US.UTF-8 is no more accurate language-wise, and most distros won't have foreign locales generated by default.
				// TODO: parse locale-archive to check whether requested host locale exists
				if _, ok := args.Env["LANG"]; !ok {
