		...
		if inFlag {
			...
			*lastStringFlag = arg
			...
			if ok {
				...
				case "-c", "--context", "-context":
					flagContext = valuePart
...
`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if FlagWantHelp || (!flagReset && containerIDp == nil) {
			cmd.Help()
			return nil
		}

		if flagReset {
			err = wormclient.WormholeReset(flagContext)
			if err != nil {
