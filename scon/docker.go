...
var (
	...
	errCloseConn = errors.New("close conn")
)
...
func (p *DockerProxy) serveConn(clientConn net.Conn) (retErr error) {
	...
	clientBufReader := bufio.NewReaderSize(clientConn, 65536)
	for {
		// read request
		...
		err = func() error {
			...
			// read response
			upstreamBufReader := bufio.NewReaderSize(upstreamConn, 65536)
