...
old_blobs="$(aws s3 ls s3://$BUCKET/blobs/ | awk '{print $4}')" || old_blobs=""
for layer in "blobs/sha256"/*; do
    hash="${layer##*/}"
    aws s3 cp $layer s3://$BUCKET/blobs/sha256:$hash --content-type application/vnd.oci.image.layer.v1.tar
done
aws s3 cp blobs/sha256/$manifest s3://$BUCKET/manifest.json --content-type application/vnd.oci.image.manifest.v1+json
for blob in $old_blobs; do
    aws s3 rm s3://$BUCKET/$blob
done
