...
mkdir -p /k3s
/usr/local/bin/k3s server --bind-address 127.0.0.1 --https-listen-port 6443 --docker --container-runtime-endpoint /var/run/docker.sock --protect-kernel-defaults --data-dir /k3s &
