...
mkdir -p /k3s
/usr/local/bin/k3s server --enable-pprof --disable metrics-server,traefik --https-listen-port 6443 --docker --container-runtime-endpoint /var/run/docker.sock --protect-kernel-defaults --flannel-backend host-gw --write-kubeconfig /run/kubeconfig.yml &
TMPDIR=/dockertmp exec /opt/orbstack-guest/simplevisor dockerd --host-gateway-ip=198.19.248.254
