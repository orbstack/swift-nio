 ...
 * Summed for all processes on the system. Tends to underestimate the real power reading by a bit.
 *
...
static int get_cru_all(coalition_t *cids, int cid_count, struct coalition_resource_usage *out_cru) {
    memset(out_cru, 0, sizeof(*out_cru));
    for (int i = 0; i < cid_count; i++) {
        struct coalition_resource_usage cru;
        int ret = coalition_info_resource_usage(cids[i], &cru, sizeof(cru));
        if (ret != 0) {
            continue;
        }

        // only do the ones we care about
        out_cru->cpu_time += cru.cpu_time;
        out_cru->cpu_ptime += cru.cpu_ptime;
        out_cru->interrupt_wakeups += cru.interrupt_wakeups;
        out_cru->platform_idle_wakeups += cru.platform_idle_wakeups;
        out_cru->energy += cru.energy;
    }

    return 0;
}

static bool contains_cid(coalition_t *cids, int cid_count, coalition_t cid) {
    for (int i = 0; i < cid_count; i++) {
        if (cids[i] == cid) {
            return true;
        }
...
int main(int argc, char *argv[]) {
    // get task_for_pid
    coalition_t cids[100000];
    int cid_i = 0;
    int cid_count = 0;
    for (int pid = 0; pid < 100000; pid++) {
        coalition_t cid = pid_to_coalition_id(pid);
        if (cid != 0) {
            // make sure this coalition is unique
            if (contains_cid(cids, cid_count, cid)) {
                continue;
            }
    ...
    // arg 1 = sampling period (if present)
    int period_sec = 1;
    bool one_shot = false;
    if (argc >= 2) {
    ...
    int ret = get_cru_all(cids, cid_count, &start_cru);
    if (ret != 0) {
    ...
    while (true) {
        ...
        int ret = get_cru_all(cids, cid_count, &new_cru);
        if (ret != 0) {
            fprintf(stderr, "coalition_info_resource_usage failed\n");
            return 1;
        }
