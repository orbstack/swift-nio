...
static void unref_inode(struct lo_data *lo, struct lo_inode *inode, uint64_t n, bool locked_for_eviction);

struct forgotten_inode {
	struct lo_inode *inode;
	struct lo_data *lo;

	forgotten_inode(struct lo_inode *inode, struct lo_data *lo)
		: inode(inode), lo(lo) {}

	~forgotten_inode() {
		// if refcount > 1, this inode is in use again, so do nothing
		// it'll be added back to the LRU once it's forgotten again
		if (inode->refcount == 1) {
			unref_inode(lo, inode, 1, true /*locked_for_eviction*/);
		}
	}
};

// _m = w/ mutex
static phmap::parallel_flat_hash_map_m<uint64_t, struct lo_inode *> inode_map;
// protected by lo mutex
static std::mutex forgotten_inodes_mutex;
//static cache::lru_cache<uint64_t, std::shared_ptr<forgotten_inode>> forgotten_inodes(9000000);
static phmap::parallel_flat_hash_map_m<uint64_t, std::shared_ptr<forgotten_inode>> forgotten_inodes;
...
static struct lo_inode *lo_inode(fuse_req_t req, fuse_ino_t ino)
{
	if (ino == FUSE_ROOT_ID)
		...
		auto inode = inode_map[ino];
		if (inode == nullptr) {
			// check forgotten LRU
			pthread_mutex_lock(&lo_data(req)->mutex);
			try {
				forgotten_inodes_mutex.lock();
				// auto forgotten = forgotten_inodes.get(ino);
				auto forgotten = forgotten_inodes[ino];
				if (forgotten == nullptr) {
					throw std::range_error("not in forgotten LRU");
				}
				forgotten_inodes_mutex.unlock();
				pthread_mutex_unlock(&lo_data(req)->mutex);
				// leave the entry. destructor takes care of the case where it's in use again
				return forgotten->inode;
			} catch (std::range_error &e) {
				// not in forgotten LRU
				pthread_mutex_unlock(&lo_data(req)->mutex);
				return nullptr;
			}
		} else {
...
static int proc_self_fd_dirfd = open("/proc/self/fd", O_RDONLY | O_DIRECTORY | O_CLOEXEC);

static uint64_t fd_to_key(int fd)
{
	char path[PATH_MAX];
	char proc_path[PATH_MAX];
	sprintf(proc_path, "%d", fd);
	
	int ret = readlinkat(proc_self_fd_dirfd, proc_path, path, PATH_MAX);
	if (ret < 0) {
		perror("readlinkat");
		abort();
	}

	return wyhash(path, ret, 0, _wyp);
}
			     ...
			     struct fuse_file_info *fi)
{
	...
	res = fstatat(ifd, "", &buf, AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW);
	if (res == -1)
			 ...
			 struct fuse_entry_param *e)
{
	...
	int ifd = lo_fd(req, parent);
	if (ifd < 0)
		return EIO;
	newfd = openat(ifd, name, O_PATH | O_NOFOLLOW);
	if (newfd == -1)
		goto out_err;
	...
	if (inode) {
		...
		inode->prev = prev;
		prev->next = inode;
		inode->key = fd_to_key(newfd);
		inode_map[inode->key] = inode;
		pthread_mutex_unlock(&lo->mutex);
	}
	e->ino = inode->key;
		    ...
		    const char *name)
{
	...
	if (ifd == -1)
		return (void) fuse_reply_err(req, EIO);
	res = linkat(AT_FDCWD, procname, ifd, name,
		     AT_SYMLINK_FOLLOW);
	if (res == -1)
		goto out_err;
...
static void lo_rmdir(fuse_req_t req, fuse_ino_t parent, const char *name)
{
	...
	if (ifd == -1)
		return (void) fuse_reply_err(req, EIO);
	res = unlinkat(ifd, name, AT_REMOVEDIR);
		      ...
		      unsigned int flags)
{
	...
	if (ifd == -1)
		return (void) fuse_reply_err(req, EIO);
	int nfd = lo_fd(req, newparent);
	if (nfd == -1)
		return (void) fuse_reply_err(req, EIO);
	res = renameat(ifd, name,
			nfd, newname);
...
static void unref_inode(struct lo_data *lo, struct lo_inode *inode, uint64_t n, bool locked_for_eviction)
{
	...
	// if (!locked_for_eviction) pthread_mutex_lock(&lo->mutex);
	// if (!locked_for_eviction && inode->refcount == 1) {
	// 	// refcount of 1 means it's already been forgotten to 0, but we kept it around in the LRU
	// 	// so ignore this overforget / reuse
	// 	pthread_mutex_unlock(&lo->mutex);
	// 	return;
	// }
	...
	// 	next->prev = prev;
	// 	prev->next = next;
	// 	inode_map.erase(inode->key);

	// 	if (!locked_for_eviction) pthread_mutex_unlock(&lo->mutex);
	// 	close(inode->fd);
	// 	free(inode);
	// } else if (inode->refcount == 1) {
	// 	// send it to LRU
	// 	inode_map.erase(inode->key);
	// 	forgotten_inodes_mutex.lock();
	// 	//forgotten_inodes.put(inode->key, std::make_shared<forgotten_inode>(inode, lo));
	// 	forgotten_inodes[inode->key] = std::make_shared<forgotten_inode>(inode, lo);
	// 	forgotten_inodes_mutex.unlock();
	// 	if (!locked_for_eviction) pthread_mutex_unlock(&lo->mutex);
	// } else {
	// 	if (!locked_for_eviction) pthread_mutex_unlock(&lo->mutex);
...
static void lo_forget_one(fuse_req_t req, fuse_ino_t ino, uint64_t nlookup)
{
	...
	unref_inode(lo, inode, nlookup, false);
}
...
static void lo_readlink(fuse_req_t req, fuse_ino_t ino)
{
	...
	if (ifd == -1)
		return (void) fuse_reply_err(req, EIO);
	res = readlinkat(ifd, "", buf, sizeof(buf));
	if (res == -1)
		return (void) fuse_reply_err(req, errno);
