...
func swext_gui_run_as_admin(shellScriptC: UnsafePointer<CChar>, promptC: UnsafePointer<CChar>)
    -> GResultErr
{
    ...
    if error != nil {
        return GResultErr(
            err: strdup(error?[NSAppleScript.errorMessage] as? String ?? "unknown error"))
    }
