...
extension DKContainer {
    ...
    var nfsPath: String {
        "\(Folders.nfsDockerContainers)/\(nameOrId)"
    }
...
enum WindowTitles {
    ...
    static func projectLogs(_ project: String) -> String {
        "\(project) – \(projectLogsBase)"
    }

    static let containerLogsBase = "Logs"
    static func containerLogs(_ container: String) -> String {
        "\(container) – \(containerLogsBase)"
    }

    static let podLogsBase = "Pod Logs"
    static func podLogs(_ pod: String) -> String {
        "\(pod) – \(podLogsBase)"
    }
}
...
enum DockerContainerLists {
    static func makeListItems(
        ...
        for (var group, containers) in composeGroups {
            let children = containers.map { DockerListItem.container($0) }.sorted(by: {
                $0.containerName < $1.containerName
            })
